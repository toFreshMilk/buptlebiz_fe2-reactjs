...
4. **기존 주석 보존**: 기존에 작성된 주석(Comments)은 명시적인 변경이나 삭제 지시가 없는 한 임의로 건드리지 말고 원본을 그대로 보존하십시오.
5. **한국어 주석 및 에러 로그 작성**: 개발자를 위한 코드 내 주석과 에러 발생 시 출력되는 로그 메시지(예: `throw new Error(...)`)는 항상 명확한 한국어로 작성하여 개발자 경험(DX)을 향상시키십시오.
...
## 8. 설정 및 에러 처리 철학 (Fail-Fast)
- **방어적 기본값 지양 (No Defensive Defaulting)**: `TenantConfig`나 필수 환경 변수 등 시스템 동작에 필수적인 설정값을 사용할 때, 호출부에서 임의의 기본값(Fallback, e.g., `|| 'default'`)을 주입하거나 옵셔널 체이닝(`?.`)으로 에러를 숨기지 마십시오.
- **Fail-Fast**: 설정이 누락되었거나 잘못되었다면 애플리케이션이 조용히 오동작하게 두는 대신, 즉시 런타임 에러를 뱉어(Fail-fast) Error Boundary가 이를 캐치하고 에러 페이지를 렌더링하도록 유도해야 합니다.
